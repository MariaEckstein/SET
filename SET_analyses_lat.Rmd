...
analysis_model = "lme"         # choose between 'aov', 'lme', 'lmer', 'hierarchicallmer'
...
if (preprocess_data) {
  ...
  allfiles = create_allfiles(inputfile_directory = preprocessing_output_dir, 
                             ...
                             data_year = data_year)
  if (!"mouse_hand" %in% colnames(allfiles)) {
  ...
  if (!"program_version" %in% colnames(allfiles)) {
...
# Lateralization label determines hemisphere, not presentation
allfiles$lateralization = factor(allfiles$lateralization, levels = c("BI", "LL", "RR", "LR", "RL"), labels = c("BI", "RRR", "LLL", "RLR", "LRL"))
# TDs!

## Add block column
# blocks = seq(22,400,20)
# for (block in blocks) {
#   if (trial > block) {
#     
#   }
...
## Add the mouse_hand column
# allfiles$mouse_hand = NA
# for (subj in unique(allfiles$subject)) {
#   subj_file = subset(allfiles, subject == subj)
#   subj_ID = subj_file$subject[1]
#   if (subj_ID %% 2) {
#     subj_file$mouse_hand = 
#   }
# }
...
if (run_regression_models) {
  ...
  contrasts(trial_data$span) = contr.poly(4)
  ...
  # ggplot(sum_data_reg,
  #        aes(across, RT, color = span, group = span)) +
  #   stat_summary(fun.data = "mean_se", geom = "pointrange") +
  #   stat_summary(fun.data = "mean_se", geom = "line") +
  #   scale_color_brewer(palette = "RdYlBu") +
  #   scale_fill_brewer(palette = "RdYlBu") +
  ...
  # Run Models
  if (analysis_model == "aov") {   # uses type-I SS because Anova(aov_mod, type = "III") doesn't work here
    
    aov_complete_formula = "log_RT ~ set * span * L_dom * across * mouse_hand + Error(Subject / (set * span * L_dom * across))"
    aov_SET_formula      = "log_RT ~ span * L_dom * across * mouse_hand + Error(Subject / (span * L_dom * across))"
    
    aov_complete = aov(as.formula(aov_complete_formula), data = sum_data_reg)
    summary(aov_complete)
    aov_SET = aov(as.formula(aov_SET_formula), data = subset(sum_data_reg, set == "SET"))
    summary(aov_SET)
    aov_noSET = aov(as.formula(aov_SET_formula), data = subset(sum_data_reg, set == "noSET"))
    summary(aov_noSET)
    
  } else if (analysis_model == "lme") {
    
    lme_com_formula = "~ (set + span + L_dom + across + mouse_hand) ^ 3"   # interactions up to third order
    lme_SET_formula = "~ (span + L_dom + across + mouse_hand) ^ 2"   # interactions up to second order
    lme_com_random  = "~ 1 | Subject/set/span/L_dom/across"
    lme_SET_random  = "~ 1 | Subject/span/L_dom/across"
    
    # Predict RTs (normal regression works)
    lme_com_form_RT = paste("RT", lme_com_formula)
    lme_SET_form_RT = paste("RT", lme_SET_formula)
    
    lme_complete_l = lme(log_RT ~ (set + span + lateralization + mouse_hand) ^ 3, ~ 1 | Subject/set/span/lateralization, data = sum_data_reg, method = "ML")
    anova.lme(lme_complete_l, type = "marginal", adjustSigma = F)
    lme_complete = lme(as.formula(lme_com_form_RT), as.formula(lme_com_random), data = subset(sum_data_reg, lateralization != "BI"), method = "ML")
    anova.lme(lme_complete, type = "marginal", adjustSigma = F)
    lme_SET = lme(as.formula(lme_SET_form_RT), as.formula(lme_SET_random), data = subset(sum_data_reg, lateralization != "BI" & set == "SET"), method = "ML")
    anova.lme(lme_SET, type = "marginal", adjustSigma = F)
    summary(lme_SET)
    lme_noSET = lme(as.formula(lme_SET_form_RT), as.formula(lme_SET_random), data = subset(sum_data_reg, lateralization != "BI" & set == "noSET"), method = "ML")
    anova.lme(lme_noSET, type = "marginal", adjustSigma = F)
    summary(lme_noSET)
    
    # Predict ACC (needs robust test or bootstrapping)
    lme_com_form_ACC = paste("ACC", lme_com_formula)
    lme_SET_form_ACC = paste("ACC", lme_SET_formula)
    
    bootsamples_com = boot(statistic = bootlme, formula = lme_com_form_ACC, random = lme_com_random, data = sum_data_reg, R = 2000)
    bootsamples_SET = boot(statistic = bootlme, formula = lme_SET_form_ACC, random = lme_SET_random, data = subset(sum_data_reg, set == "SET"), R = 2000)
    bootsamples_noSET = boot(statistic = bootlme, formula = lme_SET_form_ACC, random = lme_SET_random, data = subset(sum_data_reg, set == "noSET"), R = 2000)
    
    for (i in 1:length(bootsamples_com$t0)) {
      com_coefficient = boot.ci(bootsamples_com, type = "bca", index = i)   # bca = bias corrected and accelerated; could also calculate regular CIs with quantile() or z-scoring!
    }
    for (i in seq(1, length(bootsamples_SET$t0))) {
      SET_coefficient = boot.ci(bootsamples_SET, type = "bca", index = i)
      noSET_coefficient = boot.ci(bootsamples_noSET, type = "bca", index = i)
    }
    
  } else if (analysis_model == "lmer") {
    ...
    summary(lmer_complete)
    anova(lmer_complete, type = 3)
    ...
    Anova(glmer_noSET, type = "III")
    summary(glmer_noSET)
    
  } else if (analysis_model == "hierarchicallmer") {   # works, but takes the whole night to calculate
    
    baseline_lmer = lmer(log_RT ~ 1 + (span * L_dom * across | Subject), data = subset(trial_data, set == "SET" & ACC == 1), REML = F, na.action = na.omit, control = lmerControl(optCtrl = list(maxfun = 1e6)), verbose = T)
    span_lmer = update(baseline_lmer, .~. + span)
    L_dom_lmer = update(baseline_lmer, .~. + span + L_dom)
    across_lmer = update(baseline_lmer, .~. + span + L_dom + across)
    hand_lmer = update(baseline_lmer, .~. + span + L_dom + across + mouse_hand)
    sp_x_Ld_lmer = update(baseline_lmer, .~. + span + L_dom + across + mouse_hand + span:L_dom)
    sp_x_ac_lmer = update(baseline_lmer, .~. + span + L_dom + across + mouse_hand + span:L_dom + span:across)
    ac_x_Ld_lmer = update(baseline_lmer, .~. + span + L_dom + across + mouse_hand + span:L_dom + span:across + across:L_dom)
    complete_lmer= update(baseline_lmer, .~. +(span + L_dom + across + mouse_hand) ^ 2)
  
    ANOVA = anova(baseline_lmer, span_lmer, L_dom_lmer, across_lmer, hand_lmer, sp_x_Ld_lmer, sp_x_ac_lmer, ac_x_Ld_lmer, complete_lmer)
  }
}
...
SETnoSET_pupil$SET_minus_noSET = with(SETnoSET_pupil, rel_pupil.SET - rel_pupil.noSET)
...
gg_pupil_set_diff = ggplot(SETnoSET_pupil, aes(bin_time, SET_minus_noSET, color = span, fill = span)) +
  stat_summary(fun.y = mean, geom = 'point') +
  stat_summary(fun.data = mean_se, geom = 'smooth', alpha = 0.2) +
  scale_color_brewer(palette = "RdYlBu") +
  scale_fill_brewer(palette = "RdYlBu", guide = FALSE) +
  labs(x = 'Time (s)', y = 'SET TEPR - noSET TEPR', color = ' ', fill = ' ') +
  theme(legend.position = "none") +
  geom_tile(data = tile_data, aes(x = x, y = y, width = width, height = height), inherit.aes = F, alpha = .1) +
  geom_vline(xintercept = response_prompt_time)
