...
exclude_fast_trials = F        # Should be T
...
if (data_year == "2016") {
  ...
  ## Add block column
  allfiles$block = 6
  allfiles$block[allfiles$TrialId <= 209] = 5
  allfiles$block[allfiles$TrialId <= 167] = 4
  allfiles$block[allfiles$TrialId <= 125] = 3
  allfiles$block[allfiles$TrialId <=  83] = 2
...
if (exclude_left_handers) {
  allfiles = subset(allfiles, !Subject %in% c(left_handers, 523))  # 523 was "born left-handed but parents trained him to be right-handed"
}
...
trial_data = ddply(allfiles, .(Subject, block, TrialId, set, span, lateralization, mouse_hand, program_version), summarize,
                   ACC = mean(ACC, na.rm = T),
                   RT  = median(RT, na.rm = T))
...
```{r Performance over blocks}
if (data_year == "2017") {
  trial_data$block_trial = NA
  trial_data = trial_data[order(trial_data$Subject, trial_data$set, trial_data$span, trial_data$TrialId),]  # , trial_data$lateralization
  block_trial = 0
  block = 1
  for (row in 1:nrow(trial_data)) {
    if (trial_data$block[row] == block) {
      block_trial = block_trial + 1
    } else {
      block_trial = 1
    }
    trial_data$block_trial[row] = block_trial
    block = trial_data$block[row]
  }
  
  gg_block_ACC = ggplot(trial_data, aes(block_trial, (1 - ACC) * 100, color = mouse_hand, group = mouse_hand)) +
    stat_summary(fun.data = mean_se, geom = "pointrange") +
    stat_summary(fun.data = mean_se, geom = "line") +
    labs(x = "Trial", y = "Errors (%)", color = "Mouse hand") +
    facet_grid(paste("Block", block) ~ set)
  
  gg_block_RT = gg_block_ACC
  gg_block_RT$data = subset(trial_data, ACC == 1)
  gg_block_RT = gg_block_RT +
    aes(y = RT) +
    labs(y = "RT (msec)")
  
  gg_block_ACC2 = ggplot(subset(trial_data, block != 1), aes(2 * round(block_trial/2), (1 - ACC) * 100, color = mouse_hand, group = mouse_hand)) +
    stat_summary(fun.data = mean_se, geom = "pointrange") +
    stat_summary(fun.data = mean_se, geom = "line") +
    labs(x = "Trial", y = "Errors (%)", color = "Mouse hand") +
    facet_grid( ~ set)

  gg_block_RT2 = gg_block_ACC2
  gg_block_RT2$data = subset(gg_block_ACC2$data, ACC == 1)
  gg_block_RT2 = gg_block_RT2 +
    aes(y = RT) +
    labs(y = "RT (msec)")
  
  if (ggsave_figures) {
    ggsave(file.path(figure_dir, "03gg_block_ACC.png"), gg_block_ACC)
    ggsave(file.path(figure_dir, "03gg_block_RT.png"), gg_block_RT)
    ggsave(file.path(figure_dir, "03gg_block_ACC2.png"), gg_block_ACC2, width = 6, height = 3)
    ggsave(file.path(figure_dir, "03gg_block_RT2.png"), gg_block_RT2, width = 6, height = 3)
  }
}
```
...
### Prepare data
source("FUNCTIONS/dprime.R")
lat_data = ddply(subset(trial_data, !span %in% c("0span", "2span")),
                 .(Subject, set, lateralization), summarize,
                 ACC = mean(ACC, na.rm = T))

lat_data_cor = ddply(subset(trial_data, !span %in% c("0span", "2span") & ACC == 1),
                      .(Subject, set, lateralization), summarize,
                      RT = median(RT, na.rm = T))
...
gg_ACC_lat_mouse = ggplot(trial_data, aes(lateralization, (1 - ACC) * 100, color = mouse_hand)) +
  stat_summary(fun.data = mean_se, geom = "pointrange") +
  labs(x = "", y = "Errors (%)", color = "Mouse hand") +
  facet_grid(~ set)

gg_RT_lat_mouse = gg_ACC_lat_mouse +
  aes(y = RT) +
  labs(y = "RT (msec)")
...
if (ggsave_figures) {
  ...
  ggsave(plot = gg_ACC_lat_mouse, filename = file.path(figure_dir, "05gg_ACC_lat_mouse.png"), width = one_by_two$width + 1, height = one_by_two$height)
  ggsave(plot = gg_RT_lat_mouse, filename = file.path(figure_dir, "05gg_RT_lat_mouse.png"), width = one_by_two$width + 1, height = one_by_two$height)
}
```
...
if (run_regression_models) {
  ...
  write.csv(as.data.frame(summary(lmer_complete)$coef), paste(figure_dir, "/RT_lmer_complete_summary.csv", sep = ""))
  write.csv(as.data.frame(anova(lmer_complete, type = 3)), paste(figure_dir, "/RT_lmer_complete_aov.csv", sep = ""))
  ...
  write.csv(as.data.frame(summary(lmer_SET)$coef), paste(figure_dir, "/RT_lmer_SET_summary.csv", sep = ""))
  write.csv(as.data.frame(anova(lmer_SET, type = 3)), paste(figure_dir, "/RT_lmer_SET_aov.csv", sep = ""))
  ...
  write.csv(as.data.frame(summary(lmer_noSET)$coef), paste(figure_dir, "/RT_lmer_noSET_summary.csv", sep = ""))
  write.csv(as.data.frame(anova(lmer_noSET, type = 3)), paste(figure_dir, "/RT_lmer_noSET_aov.csv", sep = ""))
  ...
  write.csv(as.data.frame(summary(glmer_complete)$coef), paste(figure_dir, "/ACC_glmer_complete_summary.csv", sep = ""))
  write.csv(as.data.frame(anova(glmer_complete, type = 3)), paste(figure_dir, "/ACC_glmer_complete_aov.csv", sep = ""))
  ...
  write.csv(as.data.frame(summary(glmer_SET)$coef), paste(figure_dir, "/ACC_glmer_SET_summary.csv", sep = ""))
  write.csv(as.data.frame(anova(glmer_SET, type = 3)), paste(figure_dir, "/ACC_glmer_SET_aov.csv", sep = ""))
  ...
  write.csv(as.data.frame(summary(glmer_noSET)$coef), paste(figure_dir, "/ACC_glmer_noSET_summary.csv", sep = ""))
  write.csv(as.data.frame(anova(glmer_noSET, type = 3)), paste(figure_dir, "/ACC_glmer_noSET_aov.csv", sep = ""))
...
### Prepare data
lat_mouse_data = ddply(subset(trial_data, !span %in% c("0span", "2span")),#, span != "0span"
                 .(Subject, set, lateralization, mouse_hand), summarize,
                 ACC = mean(ACC, na.rm = T))
lat_mouse_data_cor = ddply(subset(trial_data, ACC == 1 & !span %in% c("0span", "2span")),#span != "0span" & 
                      .(Subject, set, lateralization, mouse_hand), summarize,
                      RT = median(RT, na.rm = T))

sum_mouse_data_all = ddply(subset(trial_data, !span %in% c("0span", "2span")),
                 .(Subject, set, mouse_hand), summarize,
                 ACC = mean(ACC, na.rm = T))
sum_mouse_data_cor = ddply(subset(trial_data, ACC == 1 & !span %in% c("0span", "2span")),
                 .(Subject, set, mouse_hand), summarize,
                 RT = median(RT, na.rm = T))
