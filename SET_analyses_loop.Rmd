...
```{r Preliminaries, echo = F, results = F, warning = F, message = F}
...
exclude_poor_subjects   = T      # If FALSE, only data is excluded that would otherwise lead to problems with data analysis (acc < 20% in any span). If TRUE, data from 
sum = 0
for (i in 0:14) {
...
```{r Read in and fix pupil data, echo = F, results = F, warning = F, message = F}
...
if (preprocess_data) {
  ...
  test_file = read.csv(file = file.path(file_dir, filenames[1]), header = T, na.strings = c("", "NA", "-1"))
  test_file = ddply(subset(test_file, TrainorExp != "Train"), .(Subject, TrialId, ACC, RT, Category, SETornoSET), summarize, RT2 = mean(RT))
  colnames(test_file)[colnames(test_file) == "RT"] = 'rt'
  colnames(test_file)[colnames(test_file) == "RT"] = 'stim'
  colnames(test_file)[colnames(test_file) == "ACC"] = 'respone'
  write.csv(test_file, paste0(file_dir, "/subset/test_file.csv"), row.names = F)
  ...
  ## Put all subject files into a single dataframes
...
gg_test_allfiles_rel = ggplot(allfiles_orig, aes(bin_time, rel_pupil, color = span, fill = span)) +
  ...
  facet_grid(~ oddball)
```
```{r Exclude subjects with insufficient performance, echo = F, results = F, warning = F, message = F}
### Get performance data for SETs and noSETs for each subject (BEFORE excluding subjects)
# First, get ACC for each trial
trialwise_perf_excl = ddply(allfiles_orig, .(Subject, TrialId, set, oddball, span), summarize,
                        RT = RT[1],
                        ACC = ACC[1])

# Then, average over trials to get subject average accuracies
source("FUNCTIONS/dprime.R")
exclusion_data2 = ddply(trialwise_perf_excl, .(Subject, set), summarize,
...
exclusion_data2c = ddply(trialwise_perf_excl, .(Subject), summarize,
                         RT = median(RT, na.rm = T),
                         ACC = mean(ACC, na.rm = T))
...
# Then, average over trials to get subject average accuracies
exclusion_data3 = ddply(trialwise_perf_excl, .(Subject, set, span), summarize,
                        RT = median(RT, na.rm = T),
                        ACC = mean(ACC, na.rm = T))
...
exclusion_data3c = ddply(trialwise_perf_excl, .(Subject, span), summarize,
                        RT = median(RT, na.rm = T),
                        ACC = mean(ACC, na.rm = T))
...
```
```{r Read in and fix questionnaires, echo = F, message = F, error = F}
...
qu$SET_verbalstrategy[qu$year == 2015] = qu$SET_verbalstrategy[qu$year == 2015] - 1
qu$SET_consciousrule[qu$year == 2015] = qu$SET_consciousrule[qu$year == 2015] - 1
qu$SET_remember[qu$year == 2015] = qu$SET_remember[qu$year == 2015] - 1
qu$SET_lastitem[qu$year == 2015] = qu$SET_lastitem[qu$year == 2015] - 1
qu$SET_features[qu$year == 2015] = qu$SET_features[qu$year == 2015] - 1

# Reverse questions 1 (verbalstrategy), 3 (memorize all), 4 (remember last item), 5 (allfeatures)
qu$SET_verbalstrategy = 4 - qu$SET_verbalstrategy
qu$SET_remember = 4 - qu$SET_remember
qu$SET_lastitem = 4 - qu$SET_lastitem
qu$SET_features = 4 - qu$SET_features
...
## Part 1: The effect of rule violation on pupil dilation

```{r create trialwise_perf subjwise_perf_oddball and subjwise_perf_set}

### Prepare data
## Get performance data for SETs and noSETs for each subject (AFTER excluding subjects)
# First, get performance for each trial, and pupil dilation for each item
allfiles4 = subset(allfiles, bin_time > 5.5 & bin_time <= 6)
trialwise_perf4 = ddply(allfiles4, .(Subject, TrialId, set, oddball, span), summarize,
                        RT = RT[1],
                        ACC = ACC[1],
                        dilation4 = mean(rel_pupil, na.rm = T))
allfiles3 = subset(allfiles, bin_time > 4 & bin_time <= 4.5)
trialwise_perf3 = ddply(allfiles3, .(Subject, TrialId, set, oddball, span), summarize,
                        dilation3 = mean(rel_pupil, na.rm = T))
allfiles2 = subset(allfiles, bin_time > 2.5 & bin_time <= 3)
trialwise_perf2 = ddply(allfiles2, .(Subject, TrialId, set, oddball, span), summarize,
                        dilation2 = mean(rel_pupil, na.rm = T))
allfiles1 = subset(allfiles, bin_time > 1 & bin_time <= 1.5)
trialwise_perf1 = ddply(allfiles1, .(Subject, TrialId, set, oddball, span), summarize,
                        dilation1 = mean(rel_pupil, na.rm = T))
trialwise_perf34 = merge(trialwise_perf4, trialwise_perf3, all=T)
trialwise_perf12 = merge(trialwise_perf2, trialwise_perf1, all=T)
trialwise_perf = merge(trialwise_perf12, trialwise_perf34, all=T)
trialwise_perf$IEPD2 = with(trialwise_perf, dilation2 - dilation1)
trialwise_perf$IEPD3 = with(trialwise_perf, dilation3 - dilation2)
trialwise_perf$IEPD4 = with(trialwise_perf, dilation4 - dilation3)
...
# Then, average over spans to get subject averages
subjwise_perf_oddball_ACC = ddply(trialwise_perf, .(Subject, set, span, oddball), summarize,
                                  ACC = mean(ACC, na.rm = T))
subjwise_perf_oddball_RT = ddply(subset(trialwise_perf, ACC == 1), .(Subject, set, span, oddball), summarize,
                                 RT = median(RT, na.rm = T),
                                 IEPD2 = mean(IEPD2, na.rm = T),
                                 IEPD3 = mean(IEPD3, na.rm = T),
                                 IEPD4 = mean(IEPD4, na.rm = T))
subjwise_perf_oddball = merge(subjwise_perf_oddball_ACC, subjwise_perf_oddball_RT, all = T)


subjwise_perf_set_ACC = ddply(trialwise_perf, .(Subject, set, span), summarize,
                              ACC = mean(ACC, na.rm = T))
subjwise_perf_set_RT = ddply(subset(trialwise_perf, ACC == 1), .(Subject, set, span), summarize,
                              RT = median(RT, na.rm = T))
subjwise_perf_set = merge(subjwise_perf_set_ACC, subjwise_perf_set_RT, all = T)

IEPD_helper = subset(trialwise_perf, ACC == 1)
IEPD_helper$IEPD = NA
IEPD_helper$IEPD[IEPD_helper$oddball == "noSET-3"] = IEPD_helper$IEPD3[IEPD_helper$oddball == "noSET-3"]
IEPD_helper$IEPD[IEPD_helper$oddball == "noSET-4"] = IEPD_helper$IEPD4[IEPD_helper$oddball == "noSET-4"]
subjwise_perf_set_IEPD = ddply(IEPD_helper, .(Subject, set, span), summarize,
                               IEPD = mean(IEPD, na.rm = T))
subjwise_perf_set = merge(subjwise_perf_set, subjwise_perf_set_IEPD, all = T)

## Add dprime
subjwise_perf_set_wide = reshape(subset(subjwise_perf_set, select = -c(RT, IEPD)), idvar = c("Subject", "span"), timevar = "set", direction = "wide")
subjwise_perf_set_wide$dprime = with(subjwise_perf_set_wide, dprime(hits = ACC.SET, false.alarms = 1 - ACC.noSET))
subjwise_perf_set = merge(subjwise_perf_set, subset(subjwise_perf_set_wide, select = -c(ACC.SET, ACC.noSET)))
...
### RT plot
RT_plot = ggplot(subjwise_perf_set, aes(span, RT, group = set, color = set)) +
  theme_bw() +
  stat_summary(fun.data = mean_cl_normal, fun.args = list(mult = 1), geom = "pointrange") +
  # stat_summary(fun.data = mean.cl.boot, fun.args = list(B = 2000, tr = .2, ci = .68), geom = "pointrange") +
  scale_color_manual(values = c("dark orange", "dark grey")) +
  labs(x = "", y = "RT (ms)") +
...
```
```{r Statistical tests, echo = F, warning = F, message = F}
### Prepare data
subjwise_perf_oddball$log_RT = log(subjwise_perf_oddball$RT + 1)
subjwise_perf_oddball$log_errors = log(1 - subjwise_perf_oddball$ACC + .01)
subjwise_perf_set$log_RT = log(subjwise_perf_set$RT + 1)
subjwise_perf_set$log_errors = log(1 - subjwise_perf_set$ACC + .01)

# Check skewness
(p_skew_RT = agostino.test(subjwise_perf_oddball$RT))
(p_skew_logRT = agostino.test(subjwise_perf_oddball$log_RT))
(p_skew_RT = agostino.test(subjwise_perf_set$RT))
(p_skew_logRT = agostino.test(subjwise_perf_set$log_RT))
...
with(subset(subjwise_perf_set, set == "SET"), pairwise.t.test(log_RT, span, p.adjust.method = "bonferroni"))
t.test(log_RT ~ span, data = subset(subjwise_perf_set, set == "SET" & span %in% c("1span", "3span")))  # to get the lowest t-value
with(subset(subjwise_perf_set, set == "noSET"), pairwise.t.test(log_RT, span, p.adjust.method = "bonferroni"))
t.test(log_RT ~ span, data = subset(subjwise_perf_set, set == "noSET" & span %in% c("0span", "3span")))  # to get the lowest t-value
# with(subset(subjwise_perf_oddball, oddball == "noSET-4"), pairwise.t.test(log_RT, span, p.adjust.method = "bonferroni"))

# Also compare No-SET-3 and No-SET-4 trials
t.test(log_RT ~ oddball, data = subset(subjwise_perf_oddball, set == "noSET" & span == "0span"), paired = T)
t.test(log_RT ~ oddball, data = subset(subjwise_perf_oddball, set == "noSET" & !span %in% c("0span")), paired = T)
mean(subset(subjwise_perf_oddball, oddball == "noSET-3" & !span %in% c("0span"))$RT)
mean(subset(subjwise_perf_oddball, oddball == "noSET-4" & !span %in% c("0span"))$RT)
...
# SnS4 = c( 1,  0, -1)
# nul  = c( 0,  0,  1)
# contrasts(subjwise_perf_set$oddball) = cbind(SnS4, nul) #cbind(SETvsnoSETs, noSET3vsnoSET4)
...
if (run_regression_models) {
  ACC_lmer = glmer(errors ~ set * span + (set + span | Subject),
                 data = trialwise_perf,
                 na.action = na.omit,
                 family = binomial,
...
```
```{r Big fat plot of pupil responses to oddballs, echo = F, warning = F, message = F, fig.height = 5, fig.width = 8}
...
```
```{r Plot IEPDs for rule-congruent and rule-violating items (big supplemental plot), echo = F, warning = F, message = F}
...
```
```{r Prepare data and plot IEPDs for rule-congruent and rule-violating items (paper plots)}
### Pupil dilation from item 3 to item 4 - correct and incorrect trials
# Prepare data
subjwise_perf_oddball_long = melt(subjwise_perf_oddball,
                                  measure.vars = c("IEPD2", "IEPD3", "IEPD4"), variable_name = "item")
subjwise_perf_oddball_long$bigspan = ifelse(subjwise_perf_oddball_long$span == "0span", "0span", "123span")
subjwise_perf_oddball_long$RVE = with(subjwise_perf_oddball_long, ifelse((item == "IEPD4" & oddball == "noSET-4") | (item == "IEPD3" & oddball == "noSET-3"), T, F))
subjwise_perf_oddball_long$oddball = factor(subjwise_perf_oddball_long$oddball, levels(subjwise_perf_oddball_long$oddball)[c(2, 1, 3)])

# Plot
gg_IEPD = ggplot(subjwise_perf_oddball_long, aes(oddball, value, fill = RVE)) +
  stat_summary(fun.data = mean_se, geom = "bar") +
  stat_summary(fun.data = mean_se) +
  theme_bw() +
  scale_fill_grey() +
  scale_x_discrete(labels = c("noS3", "SET", "noS4")) +
  labs(x = "", y = "IEPD (mm)") +
  theme(legend.position = "none") +
  facet_grid(bigspan ~ factor(item, labels = c("Item 2", "Item 3", "Item 4")))

if (ggsave_figures) {
  plot(gg_IEPD)
  ggsave(plot = gg_IEPD, paste(figure_dir, "/gg_IEPD.png", sep = ""), width = 4.5, height = 3)
}
```
```{r IEPD paper plot t-tests}
subjwise_perf_oddball
# t-tests
## IEPD rule-violating vs rule-congruent item (= VOE)
t.test(subset(subjwise_perf_oddball, oddball == "SET" & span == "0span")$IEPD3 -
       subset(subjwise_perf_oddball, oddball == "noSET-3" & span == "0span")$IEPD3)  # item3 0-span, SET vs noSET-3
t.test(subset(subjwise_perf_oddball, oddball == "SET" & span != "0span")$IEPD3 -
       subset(subjwise_perf_oddball, oddball == "noSET-3" & span != "0span")$IEPD3)  # item3 123-span, SET vs noSET-3
t.test(subset(subjwise_perf_oddball, oddball == "SET" & span == "0span")$IEPD4 -
       subset(subjwise_perf_oddball, oddball == "noSET-4" & span == "0span")$IEPD4)  # item4 0-span, SET vs noSET-4
t.test(subset(subjwise_perf_oddball, oddball == "SET" & span != "0span")$IEPD4 -
       subset(subjwise_perf_oddball, oddball == "noSET-4" & span != "0span")$IEPD4)  # item4 123-span, SET vs noSET-4

## Decreased IEPD after rule-violating item
t.test(subset(subjwise_perf_oddball, oddball == "SET" & span == "0span")$IEPD4 -
       subset(subjwise_perf_oddball, oddball == "noSET-3" & span == "0span")$IEPD4)  # 0-span item4, SET vs noSET-3
t.test(subset(subjwise_perf_oddball, oddball == "SET" & span != "0span")$IEPD4 -
       subset(subjwise_perf_oddball, oddball == "noSET-3" & span != "0span")$IEPD4)  # 123-span item4, SET vs noSET-3

## Difference betwen 0-span and higher spans
t.test(subset(subjwise_perf_oddball, oddball == "SET" & span == "0span")$IEPD3,
       subset(subjwise_perf_oddball, oddball == "SET" & span != "0span")$IEPD3)  # item3 SET
t.test(subset(subjwise_perf_oddball, oddball == "noSET-4" & span == "0span")$IEPD3,
       subset(subjwise_perf_oddball, oddball == "noSET-4" & span != "0span")$IEPD3)  # item3 noSET-4
t.test(subset(subjwise_perf_oddball, oddball == "noSET-3" & span == "0span")$IEPD3,
       subset(subjwise_perf_oddball, oddball == "noSET-3" & span != "0span")$IEPD3)  # item3 noSET-3
t.test(subset(subjwise_perf_oddball, oddball == "noSET-4" & span == "0span")$IEPD4,
       subset(subjwise_perf_oddball, oddball == "noSET-4" & span != "0span")$IEPD4)  # item4 noSET-4

## VOE item3 vs item4
t.test(subset(subjwise_perf_oddball, oddball == "noSET-3" & span == "0span")$IEPD3 -
       subset(subjwise_perf_oddball, oddball == "noSET-4" & span == "0span")$IEPD4)  # 0-span item3 vs item4
t.test(subset(subjwise_perf_oddball, oddball == "noSET-3" & span != "0span")$IEPD3 -
       subset(subjwise_perf_oddball, oddball == "noSET-4" & span != "0span")$IEPD4)  # 123-span item3 vs item4

## IEPD item presented AFTER rule-violating item
t.test(subset(subjwise_perf_oddball, oddball == "noSET-3" & span == "0span")$IEPD4)  # 0-span noSET-3 item4
t.test(subset(subjwise_perf_oddball, oddball == "noSET-3" & span != "0span")$IEPD4)  # 123-span noSET-3 item4
```

## Part 2: Flat vs hierarchical strategies

```{r Break participants into linear and u-shaped, echo = F, message = F, warning = F}

# Create ppg_dat
item4_dat = subset(allfiles, ACC == 1 & bin_time >= 5.5 & bin_time <= 6)
item234_dat = subset(allfiles, ACC == 1 & bin_time >= 1.5 & bin_time <= 6)
item23_dat = subset(allfiles, ACC == 1 & bin_time >= 1.5 & bin_time <= 5)

gg_ppg_pupils = ggplot(item23_dat, aes(bin_time, rel_pupil, color = span)) +
  stat_summary(fun.data = mean_cl_normal, geom = "pointrange")

# Run regression for each participant
if (preprocess_data) {
  
  ppgs = data.frame(Subject = unique(item234_dat$Subject), lin_b = NA, lin_p = NA, qua_b = NA, qua_p = NA, bin_b = NA, bin_p = NA, trial_b = NA, trial_p = NA, n_rows = NA)
  for (subj in item234_dat$Subject) {
    
    item234_subj = subset(item234_dat, Subject == subj)
    contrasts(item234_subj$span) = contr.poly(4)
    ppg_mod = lm(rel_pupil ~ span + bin_time + TrialId,
                 data = item234_subj, na.action = na.omit)
    coefs = as.data.frame(summary(ppg_mod)$coefficients)
    lin_coefs = coefs["span.L", c("Estimate", "Pr(>|t|)")]
    qua_coefs = coefs["span.Q", c("Estimate", "Pr(>|t|)")]
    bin_coefs = coefs["bin_time", c("Estimate", "Pr(>|t|)")]
    trial_coefs = coefs["TrialId", c("Estimate", "Pr(>|t|)")]
    
    ppgs[ppgs$Subject == subj, c("lin_b", "lin_p")] = lin_coefs
    ppgs[ppgs$Subject == subj, c("qua_b", "qua_p")] = qua_coefs
    ppgs[ppgs$Subject == subj, c("bin_b", "bin_p")] = bin_coefs
    ppgs[ppgs$Subject == subj, c("trial_b", "trial_p")] = trial_coefs
    ppgs[ppgs$Subject == subj, "n_rows"] = nrow(item234_subj)
  }
  write.csv(ppgs, "ppgs234.csv", row.names = F)
} else {
  ppgs = read.csv("ppgs234.csv")
}

# Calculate continuous ppg measure and ppg factor
ppgs$ppg_cont = ppgs$lin_b - (-ppgs$qua_b)  # more linear and less inverse-u (inverse-u = -qua_b)
# ppgs$ppg = factor(ifelse(ppgs$ppg_cont > 0, "linear", "inverse-u"))  # split at 0
# ppgs$ppg = factor(ifelse(ppgs$ppg_cont > median(ppgs$ppg_cont), "linear", "inverse-u"))  # median split
ppgs$ppg = "mix"  # 3 groups
ppgs$ppg[ppgs$ppg_cont > 0.01] = "linear"
ppgs$ppg[ppgs$ppg_cont < -0.01] = "inverse-u"
summary(ppgs)

# Number of subj in each group
numb_of_lin_subj = length(subset(ppgs, ppg == "linear")$Subject)
numb_of_qua_subj = length(subset(ppgs, ppg == "inverse-u")$Subject)

if (ggsave_figures) {
  plot(gg_ppg_pupils)
  ggsave(plot = gg_ppg_pupils, paste(figure_dir, "/gg_ppg_pupils.png", sep = ""), width = 6, height = 3.5)
}
```
```{r ppg distributions}
# Histogram
gg_ppg_hist =
  ggplot(ppgs, aes(ppg_cont, fill = ppg)) +
  geom_histogram() +
  geom_vline(xintercept = median(ppgs$ppg_cont), linetype = "dotted") +
  scale_fill_grey() +
  labs(x = "Continuous measure of pupil pattern", y = "# of participants", fill = "") +
  theme_bw()

# More plots
ggplot(ppgs, aes(lin_p, qua_p, color = ppg)) +
  geom_point(position = "jitter")
ggplot(ppgs, aes(lin_b, qua_b, color = ppg)) +
  geom_point(position = "jitter")
ggplot(ppgs) +
  geom_histogram(aes(lin_b), fill = "red") +
  geom_histogram(aes(-qua_b), fill = "blue")

if (ggsave_figures) {
  plot(gg_ppg_hist)
  ggsave(plot = gg_ppg_hist, paste(figure_dir, "/Figure5_ppg_hist.png", sep = ""), width = 6, height = 3.5)
}
```
```{r Add ppg VOE IEPD to all the dataframes}
# Add ppg
ppgs$ppg[ppgs$ppg == "mix"] = NA
pupildat_g = merge(subset(pupildat, ACC == 1), ppgs, all.x = T)
subjwise_perf_set_g = merge(subjwise_perf_set, ppgs, all.x = T)
subjwise_perf_oddball_g = merge(subjwise_perf_oddball, ppgs, all.x = T)
trialwise_perf_g = merge(trialwise_perf, ppgs, all.x = T)
qu_g = merge(ddply(subjwise_perf_set_g, .(Subject), summarize, ppg_cont = ppg_cont[1], ppg = ppg[1]),
             subset(qu, select = -c(birthday, date, birthday_, testdate_, age_days)), all = T)

# # Add VOE
# subjwise_perf_g = merge(subset(subjwise_perf_g, select = c("Subject", "set", "span", "RT", "ACC", "dprime", "ppg_cont", "ppg")),  # get rid of unnecessary columns
#                         ddply(odd_idx_dat_SnS4, .(Subject, span, set), summarize, VOE = mean(value, na.rm = T)), all = T)  # summarize VOEs of item3 and item4
# 
# # Add IEPD
# IEPD_dat = subset(odd_idx_dat12, ACC == "correct" & (oddball == "noSET-3" & item == "Item3") | (oddball == "noSET-4" & item == "Item4"))
# # IEPD_dat$ACC = as.numeric(as.character(factor(IEPD_dat$ACC, labels = c(1, 0))))
# IEPD_dat_sum = ddply(IEPD_dat, .(Subject, span), summarize, IEPD = mean(pupil, na.rm = T))  # summarize VOEs of item3 and item4
# IEPD_dat_sum$set = "noSET"
# subjwise_perf_g = merge(subjwise_perf_g,
#                         IEPD_dat_sum, all = T)
# 
# ggplot(subjwise_perf_g, aes(VOE, IEPD)) +
#   geom_point() +
#   geom_smooth(method = "lm")

# Melt questionnaire data 
qu_g_long = melt(qu_g,
                 id.vars = c("Subject", "ppg_cont", "ppg", "sex", "age_years", "year"),
                 measure.vars = c("SET_verbalstrategy", "SET_consciousrule", "SET_remember", "SET_lastitem", "SET_features", "max_forward", "total_forward", "max_backward", "total_backward", "an_syn_rawscore"),
                 variable_name = "measure")

```
```{r Compare the two groups in terms of performance and questionnaires - Plots}
# Plot
## Continous ppg measure against ACC, RT, VOE, d-prime, IEPD
gg_ppg_ACC_cont = ggplot(subjwise_perf_set_g, aes(ppg_cont, 100 * (1 - ACC), color = span)) +
  geom_point() +
  scale_color_manual(values = span_colors) +
  stat_smooth(method = "lm") +
  labs(x = "Continuous measure of pupil pattern", y = "Error rate (%)", color = "") +
  facet_grid(~ set) +
  theme_bw()
gg_ppg_RT_cont = gg_ppg_ACC_cont + aes(y = RT) + labs(y = "Respone time (ms)")
gg_ppg_IEPD_cont = gg_ppg_ACC_cont + aes(y = IEPD) + labs(y = "IEPD (mm)")
gg_ppg_dprime_cont = gg_ppg_ACC_cont + aes(y = dprime) + labs(y = "dprime")
gg_ppg_qu_cont = ggplot(subset(qu_g_long, !is.na(ppg)), aes(ppg_cont, value, color = year)) +
  geom_point() +
  stat_smooth(method = "lm") +
  labs(x = "Continuous measure of pupil pattern", y = "Questions / cog. tests", color = "") +
  facet_grid(~ measure) +
  theme_bw()
...
# gg_incr234_odd_ACC1_span0_g = gg_incr234_odd_ACC1_span0
# gg_incr234_odd_ACC1_span0_g$data = subset(odd_idx_dat12_g, ACC == "correct" & bigspan == "0span")
# gg_incr234_odd_ACC1_span0_g = gg_incr234_odd_ACC1_span0_g +
#   coord_cartesian(ylim = c(-0.03, 0.13)) +
#   facet_grid(ppg ~ item)
# gg_incr234_odd_ACC1_span123_g = gg_incr234_odd_ACC1_span0_g
# gg_incr234_odd_ACC1_span123_g$data = subset(odd_idx_dat12_g, ACC == "correct" & bigspan == "123span")
# gg_incr234_odd_ACC1_span1_g = gg_incr234_odd_ACC1_span0_g
# gg_incr234_odd_ACC1_span1_g$data = subset(odd_idx_dat12_g, ACC == "correct" & span == "1span")
# gg_incr234_odd_ACC1_span2_g = gg_incr234_odd_ACC1_span0_g
# gg_incr234_odd_ACC1_span2_g$data = subset(odd_idx_dat12_g, ACC == "correct" & span == "2span")
# gg_incr234_odd_ACC1_span3_g = gg_incr234_odd_ACC1_span0_g
# gg_incr234_odd_ACC1_span3_g$data = subset(odd_idx_dat12_g, ACC == "correct" & span == "3span")
# 
# gg_incr234_odd_ACC1_span0_g2 = gg_incr234_odd_ACC1_span0
# gg_incr234_odd_ACC1_span0_g2$data = subset(odd_idx_dat12_g, ACC == "correct" & bigspan == "0span" & fill == "red")
# gg_incr234_odd_ACC1_span0_g2 = gg_incr234_odd_ACC1_span0_g2 +
#   coord_cartesian(ylim = c(-0.03, 0.13)) +
#   aes(fill = span) +
#   labs(x = "NoS3", "NoS4") +
#   facet_grid(ppg ~ span)
# gg_incr234_odd_ACC1_span123_g2 = gg_incr234_odd_ACC1_span0_g2
# gg_incr234_odd_ACC1_span123_g2$data = subset(odd_idx_dat12_g, ACC == "correct" & bigspan == "123span" & fill == "red")

## ppg groups against ACC, RT, VOE, d-prime, IEPD
gg_ppg_ACC = ggplot(subset(subjwise_perf_set_g, !is.na(ppg)), aes(span, 100 * (1 - ACC), color = ppg, group = ppg)) +
  stat_summary(fun.data = mean_se, geom = "line") +
  stat_summary(fun.data = mean_se, geom = "pointrange") +
  scale_color_manual(values = bw_colors) +
  scale_x_discrete(labels = paste0(0:3, "sp")) +
  labs(x = "", y = "Error rate (%)", color = "") +
  facet_grid(~ set) +
  theme_bw() +
  theme(legend.position = "none")
gg_ppg_RT = gg_ppg_ACC + aes(y = RT) + labs(y = "RT (ms)")
gg_ppg_IEPD = gg_ppg_ACC + aes(y = IEPD) + labs(y = "IEPD (mm)")
gg_ppg_dprime = gg_ppg_ACC + aes(y = dprime) + labs(y = "dprime")
gg_ppg_qu = gg_ppg_qu_cont + aes(x = ppg) + stat_summary(fun.data = mean_se, geom = "pointrange")
ggplot(subset(qu_g, !is.na(ppg)), aes(SET_consciousrule, SET_remember)) +
  geom_point() +
  geom_smooth(method = "lm")

if (ggsave_figures) {
  plot(gg_ppg_ACC_cont)
  ggsave(plot = gg_ppg_ACC_cont, paste(figure_dir, "/Figure8_ACC_cont.png", sep = ""), width = 6, height = 3.5)
  plot(gg_ppg_RT_cont)
  ggsave(plot = gg_ppg_RT_cont, paste(figure_dir, "/Figure8_RT_cont.png", sep = ""), width = 6, height = 3.5)
  plot(gg_ppg_IEPD_cont)
  ggsave(plot = gg_ppg_IEPD_cont, paste(figure_dir, "/Figure8_IEPD_cont.png", sep = ""), width = 6, height = 3.5)
  plot(gg_ppg_dprime_cont)
  ggsave(plot = gg_ppg_dprime_cont, paste(figure_dir, "/Figure8_dprime_cont.png", sep = ""), width = 6, height = 3.5)
  plot(gg_ppg_qu_cont)
  ggsave(plot = gg_ppg_qu_cont, paste(figure_dir, "/Figure8_qu_cont.png", sep = ""), width = 12, height = 3.5)
  plot(gg_ppg_ACC)
  ggsave(plot = gg_ppg_ACC, paste(figure_dir, "/Figure8_ACC.png", sep = ""), width = 3, height = 2)
  plot(gg_ppg_RT)
  ggsave(plot = gg_ppg_RT, paste(figure_dir, "/Figure8_RT.png", sep = ""), width = 3, height = 2)
  plot(gg_ppg_IEPD)
  ggsave(plot = gg_ppg_IEPD, paste(figure_dir, "/Figure8_IEPD.png", sep = ""), width = 3, height = 2)
  plot(gg_ppg_dprime)
  ggsave(plot = gg_ppg_dprime, paste(figure_dir, "/Figure8_dprime.png", sep = ""), width = 3, height = 2)
  plot(gg_ppg_qu)
  ggsave(plot = gg_ppg_qu, paste(figure_dir, "/Figure8_qu.png", sep = ""), width = 12, height = 3.5)
  # plot(gg_incr234_odd_ACC1_span0_g)
  # ggsave(plot = gg_incr234_odd_ACC1_span0_g, paste(figure_dir, "/Figure8_oddball0IEPD.png", sep = ""), width = 4, height = 3.5)
  # plot(gg_incr234_odd_ACC1_span123_g)
  # ggsave(plot = gg_incr234_odd_ACC1_span123_g, paste(figure_dir, "/Figure8_oddball123IEPD.png", sep = ""), width = 4, height = 3.5)
  # ggsave(plot = gg_incr234_odd_ACC1_span1_g, paste(figure_dir, "/Figure8_oddball1IEPD.png", sep = ""), width = 4, height = 3.5)
  # ggsave(plot = gg_incr234_odd_ACC1_span2_g, paste(figure_dir, "/Figure8_oddball2IEPD.png", sep = ""), width = 4, height = 3.5)
  # ggsave(plot = gg_incr234_odd_ACC1_span3_g, paste(figure_dir, "/Figure8_oddball3IEPD.png", sep = ""), width = 4, height = 3.5)
}
```
```{r Compare the two groups in terms of performance and questionnaires - Overall Models}
## Continuous
# ACC
contrasts(trialwise_perf_g$span) = contr.poly(4)
contrasts(subjwise_perf_set_g$span) = contr.poly(4)
SET_ACC_mod_cont = glmer(ACC ~ ppg_cont + span + (span | Subject),  # can't specify interaction because model will fail to converge
                 data = subset(trialwise_perf_g, set == "SET"),
                 na.action = na.omit,
                 family = binomial,
                 control = glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 1e6)))
write.csv(as.data.frame(summary(SET_ACC_mod_cont)$coefficients), file = file.path(figure_dir, "SET_ACC_mod_cont.csv"))
anova(SET_ACC_mod_cont, type = 3)

noSET_ACC_mod_cont = update(SET_ACC_mod_cont, data = subset(trialwise_perf_g, set == "noSET"))
write.csv(as.data.frame(summary(noSET_ACC_mod_cont)$coefficients), file = file.path(figure_dir, "noSET_ACC_mod_cont.csv"))

# RT
hist(log(trialwise_perf_g$RT+1))
SET_RT_mod_cont = lmer(log(RT+1) ~ ppg_cont * span + (span | Subject),
                 data = subset(trialwise_perf_g, set == "SET"),
                 na.action = na.omit,
                 control = lmerControl(optCtrl = list(maxfun = 1e6)))
write.csv(as.data.frame(summary(SET_RT_mod_cont)$coefficients), file = file.path(figure_dir, "SET_RT_mod_cont.csv"))
anova(SET_RT_mod_cont, type = 3)

noSET_RT_mod_cont = update(SET_RT_mod_cont, data = subset(trialwise_perf_g, set == "noSET"))
write.csv(as.data.frame(summary(noSET_RT_mod_cont)$coefficients), file = file.path(figure_dir, "noSET_RT_mod_cont.csv"))
anova(noSET_RT_mod_cont, type = 3)

both_RT_mod_cont = lmer(log(RT+1) ~ (ppg_cont + span + set)^2 + (span + set | Subject),
                 data = subset(trialwise_perf_g),
                 na.action = na.omit,
                 control = lmerControl(optCtrl = list(maxfun = 1e6)))
write.csv(as.data.frame(summary(both_RT_mod_cont)$coefficients), file = file.path(figure_dir, "both_RT_mod_cont.csv"))

# IEPD
hist(subjwise_perf_set_g$IEPD)
IEPD_mod_cont = lmer(IEPD ~ ppg_cont * span + (1 | Subject),
                 data = subjwise_perf_set_g,
                 na.action = na.omit,
                 control = lmerControl(optCtrl = list(maxfun = 1e6)))
write.csv(as.data.frame(summary(IEPD_mod_cont)$coefficients), file = file.path(figure_dir, "IEPD_mod_cont.csv"))
anova(IEPD_mod_cont, type = 3)

# dprime
hist(subjwise_perf_set_g$dprime)  # definitely not normal...
dprime_mod_cont = update(IEPD_mod_cont, dprime ~ .)
dprime_coefs = data.frame(summary(dprime_mod_cont)$coefficients)
dprime_coefs$p.z = 2 * (1 - pnorm(abs(dprime_coefs$t.value)))
write.csv(dprime_coefs, file = file.path(figure_dir, "dprime_mod_cont.csv"))

## Groups
# ACC
SET_ACC_mod = update(SET_ACC_mod_cont, . ~ ppg * span + (span | Subject))
write.csv(as.data.frame(summary(SET_ACC_mod)$coefficients), file = file.path(figure_dir, "SET_ACC_mod.csv"))
noSET_ACC_mod = update(noSET_ACC_mod_cont, . ~ ppg * span + (span | Subject))
write.csv(as.data.frame(summary(noSET_ACC_mod)$coefficients), file = file.path(figure_dir, "noSET_ACC_mod.csv"))

# RT
SET_RT_mod = update(SET_RT_mod_cont, . ~ ppg * span + (span | Subject))
SET_RT_coefs = data.frame(summary(SET_RT_mod)$coefficients)
SET_RT_coefs$p.z = 2 * (1 - pnorm(abs(SET_RT_coefs$t.value)))
write.csv(SET_RT_coefs, file = file.path(figure_dir, "SET_RT_mod.csv"))
noSET_RT_mod = update(noSET_RT_mod_cont, . ~ ppg * span + (span | Subject))
noSET_RT_coefs = data.frame(summary(noSET_RT_mod)$coefficients)
noSET_RT_coefs$p.z = 2 * (1 - pnorm(abs(noSET_RT_coefs$t.value)))
write.csv(noSET_RT_coefs, file = file.path(figure_dir, "noSET_RT_mod.csv"))
both_RT_mod = update(both_RT_mod_cont, . ~ (ppg + span + set)^2 + (span | Subject))
both_RT_coefs = data.frame(summary(both_RT_mod)$coefficients)
both_RT_coefs$p.z = 2 * (1 - pnorm(abs(both_RT_coefs$t.value)))
write.csv(both_RT_coefs, file = file.path(figure_dir, "both_RT_mod.csv"))

# IEPD
IEPD_mod = update(IEPD_mod_cont, . ~ ppg * span + (1 | Subject))
IEPD_coefs = data.frame(summary(IEPD_mod)$coefficients)
IEPD_coefs$p.z = 2 * (1 - pnorm(abs(IEPD_coefs$t.value)))
write.csv(IEPD_coefs, file = file.path(figure_dir, "IEPD_mod.csv"))

# dprime
dprime_mod = update(dprime_mod_cont, . ~ ppg * span + (1 | Subject))
dprime_coefs = data.frame(summary(dprime_mod)$coefficients)
dprime_coefs$p.z = 2 * (1 - pnorm(abs(dprime_coefs$t.value)))
write.csv(dprime_coefs, file = file.path(figure_dir, "dprime_mod.csv"))

```
```{r Compare the two groups in terms of performance and questionnaires - specific t-tests}

# An-Syn / Numb-Ser
t.test(value ~ ppg, data = subset(qu_g_long, measure == "an_syn_rawscore" & year == 2013))
t.test(value ~ ppg, data = subset(qu_g_long, measure == "an_syn_rawscore" & year == 2015))

ravens = summary(lm(value ~ ppg_cont, data = subset(qu_g_long, measure == "an_syn_rawscore" & year == 2013)))
ansyn = summary(lm(value ~ ppg_cont, data = subset(qu_g_long, measure == "an_syn_rawscore" & year == 2015)))
write.csv(as.data.frame(ravens$coefficients), file = file.path(figure_dir, "ravens.csv"))
write.csv(as.data.frame(ansyn$coefficients), file = file.path(figure_dir, "ansyn.csv"))

# Questionnaire
for (quest in unique(qu_g_long$measure)) {
  quest_dat = subset(qu_g_long, measure == quest & !is.na(value) & !is.na(ppg_cont))
  print(list(quest,
             t.test(value ~ ppg, data = quest_dat),
             cor.test(quest_dat$value, quest_dat$ppg_cont)))
  mod = summary(lm(value ~ ppg_cont, data = subset(qu_g_long, measure == quest)))
  write.csv(as.data.frame(mod$coefficients), file = file.path(figure_dir, paste0("quest", quest, ".csv")))  # same results as regression
}

# SET Error rates
SET_ACC_12ppg_mod_cont = glmer(ACC ~ ppg_cont + span + (span | Subject),
                 data = subset(trialwise_perf_g, set == "SET" & span %in% c("1span", "2span")),
                 na.action = na.omit,
                 family = binomial,
                 control = glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 1e6)))
write.csv(as.data.frame(summary(SET_ACC_12ppg_mod_cont)$coefficients), file = file.path(figure_dir, "SET_ACC_12ppg_mod_cont.csv"))

SET_ACC_03ppg_mod_cont = update(SET_ACC_12ppg_mod_cont, data = subset(trialwise_perf_g, set == "SET" & span %in% c("0span", "3span")))
write.csv(as.data.frame(summary(SET_ACC_03ppg_mod_cont)$coefficients), file = file.path(figure_dir, "SET_ACC_03ppg_mod_cont.csv"))

SET_ACC_12ppg_mod = update(SET_ACC_12ppg_mod_cont, . ~ ppg + span + (span | Subject))
write.csv(as.data.frame(summary(SET_ACC_12ppg_mod)$coefficients), file = file.path(figure_dir, "SET_ACC_12ppg_mod.csv"))

SET_ACC_03ppg_mod = update(SET_ACC_03ppg_mod_cont, . ~ ppg + span + (span | Subject))
write.csv(as.data.frame(summary(SET_ACC_03ppg_mod)$coefficients), file = file.path(figure_dir, "SET_ACC_03ppg_mod.csv"))

t.test(ACC ~ ppg, data=subset(subjwise_perf_set_g, set=="SET" & span=="0span"))
t.test(ACC ~ ppg, data=subset(subjwise_perf_set_g, set=="SET" & span=="1span"))
t.test(ACC ~ ppg, data=subset(subjwise_perf_set_g, set=="SET" & span=="2span"))
t.test(ACC ~ ppg, data=subset(subjwise_perf_set_g, set=="SET" & span=="3span"))

# noSET Error rates
noSET_ACC_12ppg_mod_cont = glmer(ACC ~ ppg_cont + span + (span | Subject),
                 data = subset(trialwise_perf_g, set == "noSET" & span %in% c("1span", "2span")),
                 na.action = na.omit,
                 family = binomial,
                 control = glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 1e6)))
write.csv(as.data.frame(summary(noSET_ACC_12ppg_mod_cont)$coefficients), file = file.path(figure_dir, "noSET_ACC_12ppg_mod_cont.csv"))

noSET_ACC_03ppg_mod_cont = update(noSET_ACC_12ppg_mod_cont, data = subset(trialwise_perf_g, set == "noSET" & span %in% c("0span", "3span")))
write.csv(as.data.frame(summary(noSET_ACC_03ppg_mod_cont)$coefficients), file = file.path(figure_dir, "noSET_ACC_03ppg_mod_cont.csv"))

noSET_ACC_12ppg_mod = update(noSET_ACC_12ppg_mod_cont, . ~ ppg + span + (span | Subject))
write.csv(as.data.frame(summary(noSET_ACC_12ppg_mod)$coefficients), file = file.path(figure_dir, "noSET_ACC_12ppg_mod.csv"))

noSET_ACC_03ppg_mod = update(noSET_ACC_03ppg_mod_cont, . ~ ppg + span + (span | Subject))
write.csv(as.data.frame(summary(noSET_ACC_03ppg_mod)$coefficients), file = file.path(figure_dir, "noSET_ACC_03ppg_mod.csv"))

# d'
hist(subjwise_perf_set_g$dprime)
dprime_12ppg_mod_cont = lmer(dprime ~ ppg_cont + span + (1 | Subject),
                 data = subset(subjwise_perf_set_g, set == "noSET" & span %in% c("1span", "2span")),
                 na.action = na.omit,
                 control = lmerControl(optCtrl = list(maxfun = 1e6)))
write.csv(as.data.frame(summary(dprime_12ppg_mod_cont)$coefficients), file = file.path(figure_dir, "dprime_12ppg_mod_cont.csv"))

dprime_03ppg_mod_cont = update(dprime_12ppg_mod_cont, data = subset(subjwise_perf_set_g, set == "noSET" & span %in% c("0span", "3span")))
write.csv(as.data.frame(summary(dprime_03ppg_mod_cont)$coefficients), file = file.path(figure_dir, "dprime_03ppg_mod_cont.csv"))

dprime_12ppg_mod = update(dprime_12ppg_mod_cont, . ~ ppg + span + (1 | Subject))
write.csv(as.data.frame(summary(dprime_12ppg_mod)$coefficients), file = file.path(figure_dir, "dprime_12ppg_mod.csv"))
2 * pt(2.46, 52, lower=F)

dprime_03ppg_mod = update(dprime_03ppg_mod_cont, . ~ ppg + span + (1 | Subject))
write.csv(as.data.frame(summary(dprime_03ppg_mod)$coefficients), file = file.path(figure_dir, "dprime_03ppg_mod.csv"))
2 * pt(0.59, 52, lower=F)
...
# IEPD
trialwise_IEPDs = subset(trialwise_perf_g, set == "noSET" & ACC == 1, select = c(Subject, TrialId, oddball, span, IEPD3, IEPD4, ppg, ppg_cont))
trialwise_IEPDs$IEPD = with(trialwise_IEPDs, ifelse(oddball == "noSET-3", IEPD3, IEPD4))
IEPD_mod_invu = lmer(IEPD ~ span + (1 | Subject),
                 data = subset(trialwise_IEPDs, ppg == "inverse-u"),
                 na.action = na.omit,
                 control = lmerControl(optCtrl = list(maxfun = 1e6)))
summary(IEPD_mod_invu)
IEPD_mod_lin = update(IEPD_mod_invu, data = subset(trialwise_IEPDs, ppg == "linear"))
summary(IEPD_mod_lin)

# anova(IEPD_mod_cont, type=3)
# SET_IEPD_12ppg_mod_cont = lmer(IEPD ~ ppg_cont + span + (span | Subject),
#                  data = subset(trialwise_IEPDs, span %in% c("1span", "2span")),
#                  na.action = na.omit,
#                  control = lmerControl(optCtrl = list(maxfun = 1e6)))
# write.csv(as.data.frame(summary(SET_IEPD_12ppg_mod_cont)$coefficients), file = file.path(figure_dir, "SET_IEPD_12ppg_mod_cont.csv"))
# 
# SET_IEPD_03ppg_mod_cont = update(SET_IEPD_12ppg_mod_cont, data = subset(trialwise_IEPDs, span %in% c("0span", "3span")))
# write.csv(as.data.frame(summary(SET_IEPD_12ppg_mod_cont)$coefficients), file = file.path(figure_dir, "SET_IEPD_12ppg_mod_cont.csv"))
# 
# SET_IEPD_12ppg_mod = update(SET_IEPD_12ppg_mod_cont, IEPD ~ ppg + span + (span | Subject))
# write.csv(as.data.frame(summary(SET_IEPD_12ppg_mod)$coefficients), file = file.path(figure_dir, "SET_IEPD_12ppg_mod_cont.csv"))
# 
# SET_IEPD_03ppg_mod = update(SET_IEPD_03ppg_mod_cont, IEPD ~ ppg + span + (span | Subject))
# write.csv(as.data.frame(summary(SET_IEPD_03ppg_mod)$coefficients), file = file.path(figure_dir, "SET_IEPD_03ppg_mod.csv"))

t.test(IEPD ~ ppg, data=subset(subjwise_perf_set_g, span == "0span"))
t.test(IEPD ~ ppg, data=subset(subjwise_perf_set_g, span == "1span"))
t.test(IEPD ~ ppg, data=subset(subjwise_perf_set_g, span == "2span"))
t.test(IEPD ~ ppg, data=subset(subjwise_perf_set_g, span == "3span"))

# RTs & ACCs
hist(log(subjwise_perf_set_g$RT+1))
ddply(subjwise_perf_set_g, .(ppg, span), summarize, RT = mean(RT, na.rm = T))
t.test(log(RT+1) ~ ppg, data=subset(subjwise_perf_set_g, set == "SET" & span == "0span"))
t.test(log(RT+1) ~ ppg, data=subset(subjwise_perf_set_g, set == "SET" & span == "1span"))
t.test(log(RT+1) ~ ppg, data=subset(subjwise_perf_set_g, set == "SET" & span == "2span"))
t.test(log(RT+1) ~ ppg, data=subset(subjwise_perf_set_g, set == "SET" & span == "3span"))
...
# d-prime
t.test(dprime ~ ppg, data=subset(subjwise_perf_set_g, set == "SET" & span == "0span"))
t.test(dprime ~ ppg, data=subset(subjwise_perf_set_g, set == "SET" & span == "1span"))
t.test(dprime ~ ppg, data=subset(subjwise_perf_set_g, set == "SET" & span == "2span"))
t.test(dprime ~ ppg, data=subset(subjwise_perf_set_g, set == "SET" & span == "3span"))
```
```{r Create the big pupil dilation plot and facet by ppg}

# pupildat = read.csv(paste("pupildat_nb_", data_year, ".csv", sep = ""))
# trialwise_perf = ddply(allfiles,
#                       .(Subject, span, set, TrialId), summarize,
#                       ACC = mean(ACC, na.rm = T),
#                       RT = mean(RT, na.rm = T))

# SETs only
gg_ppg = ggplot(subset(pupildat_g, !is.na(ppg) & oddball == "SET"), aes(rel.time, rel.pupil, color = span, fill = span)) +
  geom_tile(data = tile_data, aes(x = x, y = y, width = width, height = height), inherit.aes = F, fill = "white") +  #
  geom_vline(xintercept = response_prompt_time, size = 1.5) +
...
# Add response times
RT_data = ddply(subset(subjwise_perf_set_g, ACC == 1 & set == "SET" & !is.na(ppg)), .(set, span, ppg), summarize,
                RT_mean = mean.cl.boot(RT, B = 2000, tr = .1)[1],
                RT_lower = mean.cl.boot(RT, B = 2000, tr = .1)[2],
                RT_upper = mean.cl.boot(RT, B = 2000, tr = .1)[3])
RT_data$RT_mean = 6 + RT_data$RT_mean/1000
...
RT_data$y = c(rep(rtbox-3*dist, 2),
              ...
              rep(rtbox-dist, 2),
              rep(rtbox, 2))
RT_data_long = melt(RT_data, id.vars = c("span", "set", "ppg", "y"))
...
Item4_pupils = ddply(subset(pupildat_g, !is.na(ppg) & oddball == "SET"),
                     .(Subject, ppg_cont, ppg, span, oddball),
                     summarize,
...
# SETs and noSETs
gg_ppg2 = ggplot(subset(pupildat_g, !is.na(ppg)), aes(rel.time, rel.pupil, color = span, fill = span)) +
  geom_tile(data = tile_data, aes(x = x, y = y, width = width, height = height), inherit.aes = F, fill = "white") +
  geom_vline(xintercept = response_prompt_time, size = 1.5) +
  geom_text(data = tile_data, aes(x = x, y = -0.03, label = labels), inherit.aes = F) +
  geom_text(data = resp_fix_labels, aes(x = x, y = y, label = label), inherit.aes = F) +
...
pupildat_g$oddball = factor(pupildat_g$oddball, levels = c("SET", "noSET-3", "noSET-4"))
gg_ppg_odd = ggplot(subset(pupildat_g, !is.na(ppg) & span != "0span"), aes(rel.time, rel.pupil, color = oddball, fill = oddball)) +
  geom_tile(data = tile_data, aes(x = x, y = y, width = width, height = height), inherit.aes = F, fill = "white") +
  geom_vline(xintercept = response_prompt_time, size = 1.5) +
  geom_text(data = tile_data, aes(x = x, y = -0.03, label = labels), inherit.aes = F) +
  geom_text(data = resp_fix_labels, aes(x = x, y = y, label = label), inherit.aes = F) +
...
```{r Increase in pupil dilation from one item to the next, echo = F, message = F, warning = F}
...
odd_idx_dat_SnS2 = reshape(subset(odd_idx_dat_SnS, ACC == "correct", select = -ACC),
                           idvar = c("Subject", "span", "bigspan", "variable"), timevar = "oddball", direction = "wide")
...
odd_idx_dat_SnS5 = merge(item234_dat, odd_idx_dat_SnS4, by = c("Subject", "span"))
...
```
```{r Continuous relationship between linear/quadratic pupil and linear/quadratic RTs, ACC, and VOEs, echo = F}
...
```{r Regression weights for pupil dilation by span, echo = F, wearning = F, message = F}
# # Prepare data frames
...
# }
```
```{r t-values for pupil dilation by oddball, echo = F, message = F, warning = F}
# if (preprocess_data) {
#     
#   # Prepare data frame
...
s_perf_ppg_cor = merge(subjwise_perf_set,
                       subset(gdat, select = c(Subject, ppg)))
