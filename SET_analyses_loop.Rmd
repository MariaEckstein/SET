...
trialwise_perf_ACC = ddply(allfiles, .(Subject, TrialId, set, oddball, span), summarize,
                       ACC = mean(ACC, na.rm = T))
trialwise_perf_RT = ddply(subset(allfiles, ACC == 1), .(Subject, TrialId, set, oddball, span), summarize,
                       RT = median(RT, na.rm = T),
                       dilation4 = mean(rel_pupil, na.rm = T))
trialwise_perf = merge(trialwise_perf_ACC, trialwise_perf_RT, all = T)
trialwise_perf$set = factor(trialwise_perf$set, levels = c("SET", "noSET"))
trialwise_perf$oddball = factor(trialwise_perf$oddball, levels = c("SET", "noSET-3", "noSET-4"))
trialwise_perf$errors = 1 - as.numeric(as.character(trialwise_perf$ACC))
...
```
```{r Plot IEPDs for rule-congruent and rule-violating items, echo = F, warning = F, message = F}
...
# Prepare data
odd_idx_dat10 = reshape(subset(odd_idx_dat20, select = -fill),
                        v.names = "pupil", idvar = c("Subject", "TrialId", "ACC", "span", "bigspan", "oddball"), timevar = "item", direction = "wide")
...
ppg_dat = ddply(item4_dat,
               .(Subject, span, oddball, TrialId, bin_time),
               summarize,
               pupil = mean(rel_pupil, na.rm = T))

gg_ppg_pupils = ggplot(ppg_dat, aes(bin_time, pupil, color = span)) +
  stat_summary(fun.data = mean_cl_normal, geom = "pointrange")

# Run regression for each participant
if (preprocess_data) {
  
  ppgs = data.frame(Subject = unique(ppg_dat$Subject), lin_b = NA, lin_p = NA, qua_b = NA, qua_p = NA, bin_b = NA, bin_p = NA, trial_b = NA, trial_p = NA, n_rows = NA)
  for (subj in ppg_dat$Subject) {
    
    ppg_subj = subset(ppg_dat, Subject == subj)
    contrasts(ppg_subj$span) = contr.poly(4)
    ppg_mod = lm(pupil ~ span + bin_time + TrialId,
                 data = ppg_subj, na.action = na.omit)
    ...
    ppgs[ppgs$Subject == subj, "n_rows"] = nrow(ppg_subj)
  }
  write.csv(ppgs, "ppgs.csv", row.names = F)
} else {
  ppgs = read.csv("ppgs.csv")
}
...
ppgs$ppg = factor(ifelse(ppgs$ppg_cont > median(ppgs$ppg_cont), "linear", "inverse-u"))
summary(ppgs)
...
```{r ppg distributions}
# Histogram
gg_ppg_hist = ggplot(ppgs, aes(ppg_cont, fill = ppg)) +
  geom_histogram(bins = 12) +
  geom_vline(xintercept = (1/90), linetype = "dotted") +
...
```
```{r Add ppg to all the dataframes}
pupildat_g = merge(subset(pupildat, ACC == 1), ppgs, all.x = T)
subjwise_perf_g = merge(subjwise_perf_oddball, ppgs, all.x = T)
trialwise_perf_g = merge(trialwise_perf, ppgs, all.x = T)
qu_g = merge(ddply(subjwise_perf_g, .(Subject), summarize, ppg_cont = ppg_cont[1], ppg = ppg[1]),
...
# Add response times
RT_data = ddply(subset(subjwise_perf_g, ACC == 1 & oddball == "SET"), .(oddball, span, ppg), summarize,
                RT_mean = mean.cl.boot(RT, B = 2000, tr = .1)[1],
                RT_lower = mean.cl.boot(RT, B = 2000, tr = .1)[2],
                RT_upper = mean.cl.boot(RT, B = 2000, tr = .1)[3])
RT_data$RT_mean = 6 + RT_data$RT_mean/1000
...
# Compare the two groups in terms of performance and questionnaires

```{r Plots}
# Prepare data
## Add VOEs to subjwise_perf_g
subjwise_perf_g = merge(subset(subjwise_perf_g, select = c("Subject", "set", "span", "oddball", "RT", "ACC", "ppg_cont", "ppg")),  # get rid of unnecessary columns
                        ddply(odd_idx_dat_SnS4, .(Subject, span, set), summarize, VOE = mean(value, na.rm = T)), all = T)  # summarize VOEs of item3 and item 4

## Add IEPDs
IEPD_dat = subset(odd_idx_dat12, ACC == "correct" & (oddball == "noSET-3" & item == "Item3") | (oddball == "noSET-4" & item == "Item4"))
colnames(IEPD_dat)[colnames(IEPD_dat) == "pupil"] = "IEPD"
subjwise_perf_g = merge(subjwise_perf_g,
                        subset(IEPD_dat, select = -c(ACC, fill, item, bigspan)), all = T)
...
## Add d-prime
subjwise_perf_g = merge(subjwise_perf_g,
                        subset(subjwise_perf_oddball_both, select = c("Subject", "span", "oddball", "dprime")), all = T)
...
# Plot
## Continous ppg measure against ACC, RT, VOE, d-prime
gg_ppg_ACC_cont = ggplot(subjwise_perf_g, aes(ppg_cont, 100 * (1 - ACC), color = span)) +
  geom_point() +
  scale_color_manual(values = span_colors) +
...
gg_ppg_VOE_cont = gg_ppg_ACC_cont + aes(y = VOE) + labs(y = "VOE (mm)")
gg_ppg_IEPD_cont = gg_ppg_ACC_cont + aes(y = IEPD) + labs(y = "IEPD (mm)")
gg_ppg_dprime_cont = gg_ppg_ACC_cont + aes(y = dprime) + labs(y = "dprime")
gg_ppg_qu_cont = ggplot(qu_g_long, aes(ppg_cont, value, color = year)) +
  geom_point() +
  stat_smooth(method = "lm") +
  labs(x = "Continuous measure of pupil pattern", y = "Questions / cog. tests", color = "") +
  facet_grid(~ measure)

## ppg groups against ACC, RT, VOE, d-prime
gg_ppg_ACC = ggplot(subjwise_perf_g, aes(span, 100 * (1 - ACC), color = ppg)) +
...
gg_ppg_VOE = gg_ppg_ACC + aes(y = VOE) + labs(y = "VOE (mm)")
...
if (ggsave_figures) {
  ...
  plot(gg_ppg_VOE_cont)
  ggsave(plot = gg_ppg_VOE_cont, paste(figure_dir, "/Figure8_VOE_cont.png", sep = ""), width = 6, height = 3.5)
  ...
  plot(gg_ppg_VOE)
  ggsave(plot = gg_ppg_VOE, paste(figure_dir, "/Figure8_VOE.png", sep = ""), width = 6, height = 3.5)
  plot(gg_ppg_IEPD)
...
contrasts(subjwise_perf_g$span) = contr.poly(4)
SET_ACC_mod_cont = glmer(ACC ~ ppg_cont + span + (span | Subject),
                 data = subset(trialwise_perf_g, set == "SET"),
                 na.action = na.omit,
                 family = binomial,
                 control = glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 1e6)))
summary(SET_ACC_mod_cont)

noSET_ACC_mod_cont = update(SET_ACC_mod_cont, data = subset(trialwise_perf_g, set == "noSET"))
summary(noSET_ACC_mod_cont)
...
SET_RT_mod_cont = lmer(log(RT+1) ~ ppg_cont + span + (span | Subject),
                 ...
                 na.action = na.omit,
                 control = lmerControl(optCtrl = list(maxfun = 1e6)))
summary(SET_RT_mod_cont)

noSET_RT_mod_cont = update(SET_RT_mod_cont, data = subset(trialwise_perf_g, set == "noSET"))
summary(noSET_RT_mod_cont)

# VOE
hist(subjwise_perf_g$VOE)
VOE_mod_cont = lmer(VOE ~ ppg_cont + span + (1 | Subject),
                 data = subjwise_perf_g,
                 na.action = na.omit,
                 control = lmerControl(optCtrl = list(maxfun = 1e6)))
summary(VOE_mod_cont)

# dprime
hist(subjwise_perf_g$dprime)  # definitely not normal...
dprime_mod_cont = update(VOE_mod_cont, dprime ~ .)
...
SET_RT_coefs  # summary(SET_RT_mod)
...
noSET_RT_coefs  # summary(noSET_RT_mod)

# VOE
VOE_mod = update(VOE_mod_cont, . ~ ppg + span + (1 | Subject))
VOE_coefs = data.frame(summary(VOE_mod)$coefficients)
VOE_coefs$p.z = 2 * (1 - pnorm(abs(VOE_coefs$t.value)))
VOE_coefs
...
## Correlations
sub_dat = subset(subjwise_perf_g, set == "SET")
cor.test(sub_dat$ACC, sub_dat$ppg_cont)
cor.test(log(sub_dat$RT+1), sub_dat$ppg_cont)
sub_dat = subset(subjwise_perf_g, set == "noSET")
cor.test(sub_dat$ACC, sub_dat$ppg_cont)
cor.test(log(sub_dat$RT+1), sub_dat$ppg_cont)
```
...
colnames(odd_idx_dat_SnS3)[colnames(odd_idx_dat_SnS3) == "variable"] = c("item", "VOEitem")
odd_idx_dat_SnS4 = subset(odd_idx_dat_SnS3, (item == "pupil.3" & VOEitem == "VOE3") | (item == "pupil.4" & VOEitem == "VOE4"))
odd_idx_dat_SnS5 = merge(ppg_dat, odd_idx_dat_SnS4, by = c("Subject", "span"))
