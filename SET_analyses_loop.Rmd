...
```{r, echo = F, results = F, warning = F, message = F, fig.height = 8, fig.width = 8}
...
dprime_plot = ggplot(subjwise_perf_oddball_both, aes(span, dprime, fill = span, color = span)) +
  # stat_summary(fun.data = mean.cl.boot, geom = "bar", B = 2000, tr = .2) +
  theme_bw() +
  # stat_summary(fun.y = mean, geom = "line", aes(group = 1), color = "black") +
  # stat_summary(fun.data = mean.cl.boot, fun.args = list(B = 2000, tr = .2), geom = "pointrange") +
...
RT_plot = ggplot(cor_subjwise_perf_set, aes(span, RT, group = set, color = set)) +  #, color = span, fill = span, shape = set
  theme_bw() +
...
if (ggsave_figures) {
  ACC_and_RT_plot = arrangeGrob(ACC_plot, RT_plot, dprime_plot, nrow = 3)
  plot(ACC_and_RT_plot)
  ggsave(paste(figure_dir, "/Figure3_.png", sep = ""), ACC_and_RT_plot, width = 4, height = 8)
  
  ggsave(paste(figure_dir, "/Figure3acc_bw_.png", sep = ""), ACC_plot_bw, width = one_by_two$width, height = one_by_two$height)
  ggsave(paste(figure_dir, "/Figure3rt_bw_.png", sep = ""), RT_plot_bw, width = 3.5, height = 2.5)
  
  # ACC_and_RT_plot2 = arrangeGrob(ACC_plot2, RT_plot2, dprime_plot2, nrow = 3)
  # plot(ACC_and_RT_plot2)
  # ggsave(paste(figure_dir, "//Figure3s_.png", sep = ""), ACC_and_RT_plot2, width = 6, height = 8)
}
```
...
t.test(log_errors ~ span, data = subset(subjwise_perf_set, set == "SET" & span %in% c("0span", "3span")))  # to get the lowest t-value
with(subset(subjwise_perf_set, set == "noSET"), pairwise.t.test(log_errors, span, p.adjust.method = "bonferroni"))
t.test(log_errors ~ span, data = subset(subjwise_perf_set, set == "noSET" & span %in% c("1span", "2span")))  # to get the lowest t-value
...
## Splitting participants into linear and u-shaped
...
if (ggsave_figures) {
  plot(gg_VOE)
  plot(gg_VOE_I34)
  ggsave(plot = gg_VOE, paste(figure_dir, "/Figure7_VOE_.png", sep = ""), width = 3, height = 4)
  ggsave(plot = gg_VOE_I3, paste(figure_dir, "/Figure7_VOE_I34_.png", sep = ""), width = 3, height = 4)
}
...
### RTs
gg_ppg_RT = ggplot(s_perf_ppg, aes(span, RT, color = ppg, group = ppg, shape = ppg)) +
  theme_bw() +
  stat_summary(fun.data = mean_cl_normal, fun.args = list(mult = 1), geom = "pointrange") +
  stat_summary(fun.data = mean_cl_normal, fun.args = list(mult = 1), geom = "line") +
  scale_color_manual(values = bw_colors) +
  ...
  scale_x_discrete(labels = c("0sp", "1sp", "2sp", "3sp")) +
  theme(legend.position = "none") +  #c(.1, .9), legend.background = element_rect(fill = "transparent")) +
  facet_grid( ~ set)
### ACCs
gg_ppg_ACC = ggplot(s_perf_ppg, aes(span, 1-ACC, color = ppg, group = ppg, shape = ppg)) +
  ...
  scale_color_manual(values = bw_colors) +
  # coord_trans(y = "log10") +
  # scale_y_log10(breaks = seq(0, .20, .02)) +  #Gives the wrong results!!!
...
gg_linqua_RT_cont = ggplot(s_perf_ppg, aes(lin_minus_qua, RT/1000, color = oddball, fill = oddball)) +
  theme_bw() +
  geom_vline(xintercept = 0) +
  geom_point(alpha = .5) +
  geom_smooth(method = "lm") +
...
if (ggsave_figures) {
  plot(gg_ppg_RT)
  ggsave(plot = gg_ppg_RT, paste(figure_dir, "/Figure5_ppg_RT.png", sep = ""), width = one_by_two$width, height = one_by_two$height)
  plot(gg_ppg_ACC)
  ggsave(plot = gg_ppg_ACC, paste(figure_dir, "/Figure5_ppg_ACC.png", sep = ""), width = one_by_two$width, height = one_by_two$height)
  ...
  ggsave(plot = gg_ppg_dprime, file.path(figure_dir, "Figure5_ppg_dprime.png"), width = one_by_one$width, height = one_by_one$height)
  ggsave(plot = gg_ppg_ACC_SET_minus_noSET, file.path(figure_dir, "Figure5_ppg_ACC_SET_minus_noSET.png"), width = one_by_one$width, height = one_by_one$height)
}
```
...
subjwise_SET_perf = ddply(subset(trialwise_perf, oddball == "SET"), .(Subject), summarize,
                          ...
                          SET_ACC = mean(ACC, na.rm = T))

subjwise_SET_perf$ACC_group = "96-100%"
subjwise_SET_perf$ACC_group[subjwise_SET_perf$SET_ACC < .95] = "65-95%"
...
pupildat_SET_ACC1$ACC_group = factor(pupildat_SET_ACC1$ACC_group)
pupildat_SET_ACC1$RT_group = factor(pupildat_SET_ACC1$RT_group)

# Get number of Ss in each group
print(levels(pupildat_SET_ACC1$ACC_group)[1])
length(unique(subset(pupildat_SET_ACC1, ACC_group == levels(pupildat_SET_ACC1$ACC_group)[1])$Subject))
print(levels(pupildat_SET_ACC1$ACC_group)[2])
length(unique(subset(pupildat_SET_ACC1, ACC_group == levels(pupildat_SET_ACC1$ACC_group)[2])$Subject))
...
pupildat_SET_ACC1_good$RT_group = factor(pupildat_SET_ACC1_good$RT_group)
...
# Get number of Ss in each group
print(levels(pupildat_SET_ACC1_good$RT_group)[1])
length(unique(subset(pupildat_SET_ACC1_good, RT_group == levels(pupildat_SET_ACC1_good$RT_group)[1])$Subject))
print(levels(pupildat_SET_ACC1_good$RT_group)[2])
length(unique(subset(pupildat_SET_ACC1_good, RT_group == levels(pupildat_SET_ACC1_good$RT_group)[2])$Subject))
...
if (ggsave_figures) {
  ...
  plot(gg_fs_inlay)
  plot(gg_bfs_inlay)
  ggsave(plot = gg_better_worse_pupils, paste(figure_dir, "/gg_better_worse_pupils.png", sep = ""), width = 9, height = 5)
  ggsave(plot = gg_fast_slow_pupils, paste(figure_dir, "/gg_fast_slow_pupils.png", sep = ""), width = 9, height = 5)
  ggsave(plot = gg_better_fast_slow_pupils, paste(figure_dir, "/gg_better_fast_slow_pupils.png", sep = ""), width = 9, height = 5)
  ggsave(plot = gg_bw_inlay, paste(figure_dir, "/gg_bw_inlay.png", sep = ""), width = 3, height = 2.5)
  ggsave(plot = gg_fs_inlay, paste(figure_dir, "/gg_fs_inlay.png", sep = ""), width = 3, height = 2.5)
  ggsave(plot = gg_bfs_inlay, paste(figure_dir, "/gg_bfs_inlay.png", sep = ""), width = 3, height = 2.5)
}
...
trialwise_above = subset(trialwise_above, ACC_group == "96-100%")
subjwise_above  = ddply(trialwise_above, .(Subject, set, span), summarize,
                        RT = mean(RT, na.rm = T))
spanwise_above  = ddply(subjwise_above, .(set, span), summarize,
                        mean_RT   = mean(RT, na.rm = T),
